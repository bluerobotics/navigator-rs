...
impl Navigator {
    ...
    pub fn init(&mut self) {
        // Initialize devices that require it
        for device in &mut self.devices {
            if let Some(mag) = device.as_any_mut().downcast_mut::<Ak09915Device>() {
                mag.init().unwrap();
            } else if let Some(imu) = device.as_any_mut().downcast_mut::<Icm20689Device>() {
                imu.init().unwrap();
            } else if let Some(bmp) = device.as_any_mut().downcast_mut::<Bmp280Device>() {
                bmp.init().unwrap();
            } else if let Some(adc) = device.as_any_mut().downcast_mut::<Ads1115Device>() {
                adc.init().unwrap();
            }
            // Initialize other devices as needed
        }
    }

    pub fn self_test(&mut self) -> bool {
        // Implement self-test logic if necessary
        true
    }
